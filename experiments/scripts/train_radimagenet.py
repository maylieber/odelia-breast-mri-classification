"""
Training script for the LR-tuned frozen-RadImageNet run: RadImageNet
ResNet50, backbone still fully frozen (unchanged from the frozen-
RadImageNet baseline / the augmented frozen-RadImageNet run), same
aug-on/dropout-0.5/weight-decay 1e-4/10-epoch head recipe as the augmented
frozen-RadImageNet run, but lr 1e-5 -> 1e-4.
"""
...
from model_radimagenet import BreastMRINetworkRadImageNet
...
# --------------------------------------------------
# Data (augmented train split, matches the reference configuration's loaders)
# --------------------------------------------------
...
# the augmented frozen-RadImageNet run's recipe, only lr changed: frozen
# backbone (unchanged), aug on, dropout 0.5 (baked into
# model_radimagenet.py's Classifier), weight decay 1e-4, single LR 1e-4
