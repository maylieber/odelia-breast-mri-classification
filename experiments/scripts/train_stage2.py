"""
Two-stage cascade, Stage 2: Benign vs. Malignant, trained and evaluated
only on lesion-positive patients (Lesion in {1, 2}).

Second half of the two-stage cascade requested as a variant of the
reference configuration: same architecture/hyperparameters as the
reference configuration and as Stage 1 (train_stage1.py) -- ResNet18,
layer4-only unfrozen, plain attention pooling, lr=1e-5, weight_decay=1e-4,
dropout 0.5, batch 16, augmentation on, 15 epochs, T2/Pre/Post1,
only the data subset and binary target change
(Benign vs. Malignant instead of No-lesion vs. Lesion). eval_hierarchical.py
chains this stage's checkpoint with Stage 1's back into a 3-way prediction.
...
from load_dataset_hierarchical import create_stage2_dataloaders
...
plt.plot(fpr, tpr, label=f"Malignant vs. Benign (AUC = {overall_auc:.3f})")
