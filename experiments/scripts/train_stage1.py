"""
Two-stage cascade, Stage 1: No lesion vs. Lesion (Benign + Malignant
collapsed).

First half of the two-stage cascade requested as a variant of the
reference configuration: same architecture/hyperparameters as the
reference configuration (ResNet18, layer4-only unfrozen, plain attention
pooling, lr=1e-5, weight_decay=1e-4, dropout 0.5, batch 16, augmentation
on, 15 epochs, T2/Pre/Post1) -- only the classification target changes,
from 3-way to this stage's binary No-lesion/Lesion split. Stage 2
(train_stage2.py) handles Benign vs. Malignant on lesion-positive
patients only; eval_hierarchical.py chains both stages' checkpoints back
into a 3-way prediction for a direct comparison against the reference
configuration.

train.py / model.py / dataset.py / load_dataset.py are untouched.
"""
...
from load_dataset_hierarchical import create_stage1_dataloaders
from model_hierarchical import BreastMRINetworkHierarchical

# so output paths resolve regardless of the working directory this script
# is invoked from
...
train_loader, val_loader, test_loader, class_weights = create_stage1_dataloaders()
...
# single LR for all trainable params -- matches the reference configuration exactly
...
# --------------------------------------------------
# Sensitivity / specificity / AUC (positive class = Lesion)
# --------------------------------------------------
...
for i, name in enumerate(CLASS_NAMES):
    ...
    binary_labels = (test_labels_arr == i).astype(int)
    auc = roc_auc_score(binary_labels, test_probabilities[:, i])
...
print(f"\nAUC (Lesion vs. No lesion): {overall_auc:.4f}")
...
fpr, tpr, _ = roc_curve(test_labels_arr, test_probabilities[:, 1])

plt.plot(fpr, tpr, label=f"Lesion vs. No lesion (AUC = {overall_auc:.3f})")
plt.plot([0, 1], [0, 1], linestyle="--", color="gray", label="Chance")

plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate")
plt.title(f"ROC curve (test set) ({RUN_NAME})")
plt.legend()
plt.tight_layout()
